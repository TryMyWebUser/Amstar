...
class User
{
    ...
    public static function setRProducts($title, $img, $cate)
    {
        $conn = Database::getConnect();
        $targetDir = "../uploads/RProducts/"; // Define your upload directory
        ...
        // Insert data into database
        $sql = "INSERT INTO `r-product` (`img`, `title`, `category`, `created_at`) 
                VALUES ('$filePath', '$title', '$cate', NOW())";

        if ($conn->query($sql)) {
            header("Location: viewRProduct.php");
            exit;
        } else {
            return "Error occurred while saving data: " . $conn->error;
        }
    }
    public static function updateRProducts($title, $img, $cate, $getID, $conn)
    {
        $targetDir = "../uploads/RProducts/"; // Define your upload directory
        ...
        $qry = $conn->query("SELECT * FROM `r-product` WHERE `id` = '$getID'")->fetch_array();
        ...
        if (!empty($_FILES["img"]["name"])) {
            ...
            // Update database with new image path
            $sql = "UPDATE `r-product` SET `img` = ?, `title` = ?, `category` = ?, `created_at` = NOW() WHERE `id` = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssi", $filePath, $title, $cate, $getID);
        } else {
            // Update database without changing the image
            $sql = "UPDATE `r-product` SET `title` = ?, `category` = ?, `created_at` = NOW() WHERE `id` = ?";
            $stmt = $conn->prepare($sql);
